...
class PlayerScoreTestCase(BaseGameTestCase):
    ...
    def test_score_calculation(self):
        ...
        PlayerAnswer.objects.create(
            player=self.player1, question=self.q3, is_correct=False
        )
        ...
        annotated_player = (
            self.player1.__class__.objects.filter(id=self.player1.id)
            .with_scores()
            .first()
        )
...
class TeamScoreTestCase(BaseGameTestCase):
    ...
    def test_team_total_score(self):
        ...
        PlayerAnswer.objects.create(
            player=self.player3, question=self.q3, is_correct=True
        )  # 300
